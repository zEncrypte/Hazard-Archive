...
def StatusChanger(token, Status):
    ...
    try:
        requests.patch("https://discord.com/api/v10/users/@me/settings", proxies=proxy(), headers=getheaders(token), json=custom_status)
...
def BioChanger(token, bio):
    ...
    try:
        requests.patch("https://discord.com/api/v10/users/@me", proxies=proxy(), headers=getheaders(token), json=custom_bio)
